  ...
  def stats
    ...
    if params.has_key?(:teams)
      if params[:teams][:range].to_i==0
        teams = Top.where(:formato => 'VGC19-Moon').where.not(:tipo_torneo => 'Otro')
      else
        teams = Top.where(:formato => 'VGC19-Moon').where.not(:tipo_torneo => 'Otro').where(:fecha=>DateTime.now-params[:teams][:range].to_i..DateTime.now)
      end
    else 
        teams = Top.where(:formato => 'VGC19-Moon').where.not(:tipo_torneo => 'Otro')
    end
